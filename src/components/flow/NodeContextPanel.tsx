...
async function generateNodeContext(nodeData: HistoricalNodeData) {
  try {
    const {
      data,
      error
    } = await supabase.functions.invoke("analyze-node", {
      body: {
...
export function NodeContextPanel({
  ...
  return <SidebarProvider>
      {/* A full-width flex container that right-aligns its children */}
      <div className="w-full flex flex-col items-end">
        <Sidebar className="w-full">
          ...
          <SidebarContent className="w-full p-4 text-right mx-[240px] my-0 py-[16px] px-[22px]">
            <ScrollArea className="h-[calc(100vh-120px)] mx-[240px]">
              {isLoading ? <div className="space-y-4">
                  <Skeleton className="h-4 w-3/4 mx-auto" />
                  <Skeleton className="h-4 w-full mx-auto" />
                  <Skeleton className="h-4 w-2/3 mx-auto" />
                </div> : error ? <div className="text-red-500">
                  Failed to load context. Please try again later.
                </div> : <div className="prose prose-sm max-w-none">{context}</div>}
            </ScrollArea>
          </SidebarContent>
        </Sidebar>
      </div>
